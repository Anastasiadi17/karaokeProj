...
#
# ВАЖНО, ЧЕГО ЭТОТ ПОРОГ НЕ ЗНАЧИТ. Оба замера сняты с ОДНОГО входа — того
# самого синуса, который тест и генерирует. На другом синтетическом сигнале
# (аккорд 110/164.8/220 Гц плюс частотно-модулированная линия 440 Гц, 20 с,
# замер 13 августа 2026) ошибка составила 0.091 — почти вдвое выше порога.
# Величина расхождения зависит от материала, поэтому «модель восстанавливает
# микс с точностью 5%» из этой константы НЕ следует: она означает лишь, что
# на собственном сигнале теста расхождение стабильно и мало.
#
# Отсюда правило: порог не поднимать «на всякий случай» — он единственный,
# кто ловит потерянную транспозицию и забытую денормировку. Если понадобится
# утверждение о качестве модели, мерить на нескольких разных входах и лучше
# на настоящей музыке, а не на синтетике.
