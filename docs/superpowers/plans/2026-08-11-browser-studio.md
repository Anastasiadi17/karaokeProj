...
`server.proxy` ниже прокидывает `/api` на бэкенд подсистемы A (порт 8000 —
штатный порт `uvicorn` из README API, раздел «Запуск»). Так `ApiClient` может
оставаться на относительных путях (`baseUrl = ""`, см. Step 6): браузер и API
всегда видятся одним origin — в деве через этот прокси, в проде за общим
обратным прокси. CORS осознанно не используется: раз кросс-доменных запросов
нет вовсе, добавлять `CORSMiddleware` в API незачем — это расширило бы
поверхность атаки без выгоды.
