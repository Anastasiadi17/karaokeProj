...
**Отменено более поздним замером (волна правок финального ревью, I10).**
Оба числа выше — и тёплые ≈24 с, и холодные ≈167 с — получены линейной
экстраполяцией с 30-секундного клипа, а она умножает постоянную составляющую
(загрузку весов) вместе с переменной. Контрольный прогон на предельных 600 с
дал 20,9 с там, где пропорция обещала ≈70 с. По двум точкам: ≈2,6 с
постоянных издержек + ≈0,0305 с на секунду звука, то есть трек 3,5 мин — ≈9 с
прогретым процессом и ≈29 с холодным (≈20 с одноразовой JIT-компиляции ядер
от длины аудио не зависят и на 7 не умножаются). Раздел 4.5 переписан по
двухточечной модели; денежные таблицы 4.5/4.6 оставлены на 25 с намеренно —
новое знание двигает себестоимость только вниз, а пересчёт тарифной модели
