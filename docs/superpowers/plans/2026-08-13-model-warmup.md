...
Run: `grep -rn "загружается один раз при старте\|модель уже в памяти" docs/ api/ --include=*.md --include=*.py | grep -v "2026-08-13-model-warmup"`
Expected: совпадений нет. Исключение обязательно: спека и план этой задачи цитируют старую формулировку, чтобы объяснить, что именно было неверно, — это не остатки обещания.
