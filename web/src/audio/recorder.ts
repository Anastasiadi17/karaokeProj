...
export class Recorder {
  ...
  async start(stream: MediaStream): Promise<void> {
    // Прошлый узел отцепляется здесь, а не только снаружи: его обработчик
    // держит ссылку на записывающий объект и продолжил бы досыпать кадры в
    // только что очищенные буферы, сплетая два дубля в один.
    this.stop();
